"""Dorai enhancer ORT inference helper for MATLAB's py.* bridge.

The self-contained enhancer ORT flatbuffer maps a raw multi-channel mic array
...
def load(path):
    """Open the enhancer ORT flatbuffer model and cache the ORT session."""
    key = str(path)
...
def enhance(path, mic_flat, n_samples, n_mics):
    ...
    L = int(np.asarray(n_samples).item())
    N = int(np.asarray(n_mics).item())
...
def enhance_metered(path, mic_flat, n_samples, n_mics,
    ...
    Times the actual ORT inference to get the real-time factor (RTF), then
    ...
    L = int(np.asarray(n_samples).item())
    N = int(np.asarray(n_mics).item())
    board_w = float(np.asarray(board_w).item())
    threshold_mw = float(np.asarray(threshold_mw).item())
    flat = np.asarray(mic_flat, dtype=np.float32).reshape(-1)
    if flat.size != L * N:
        raise ValueError(
