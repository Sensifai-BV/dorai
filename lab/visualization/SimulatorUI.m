...
%SIMULATORUI  Dorai acoustic simulator — file-based mix + ONNX denoise UI.
%
...
%     2. Pick a scene preset (dog distance / position).
%     3. "Play Mixed" — mix speech + factory + welding noise through the
%        physical N-mic array and play it back with live per-mic
%        waveforms and a live spectrogram of the noisy mix.
%     4. "Clean Speech" — feed the noisy N-mic array to dorai.ort, play
...
%     5. Latest-recording grid plays back each captured WAV.
    ...
    methods
        function obj = SimulatorUI(cfg, geo, audio, enhancer)
            ...
            obj.factory_gain = cfg.factory_gain_init;
            obj.welding_gain = cfg.welding_gain_init;
        ...
        function start(obj)
            fprintf('[Dorai] Ready — pick a sample, then Play Mixed / Clean.\n');
            waitfor(obj.fig);
        end
    end
    ...
    methods (Access = private)
        ...
        function build_control_panel_(obj, cpw)
            ...
            uicontrol(ctrl, 'Style', 'text', 'String', 'Dorai', ...
            ...
            obj.H.lbl_dg = uicontrol(ctrl, 'Style', 'text', ...
                'String', sprintf('Factory noise  %.2f', obj.factory_gain), ...
            ...
            obj.H.btn_clean = uicontrol(ctrl, 'Style', 'togglebutton', ...
                'String', sprintf('%c Clean Speech', 9658), ...
                'FontSize', 10, 'FontWeight', 'bold', ...
            ...
            for m = 1:cfg.n_mics
                ...
                if m == 1
                    title(ax, sprintf(['Physical %d-Mic Array  ' ...
                        '(speech + factory + welding, room reflections)'], ...
                        cfg.n_mics), ...
                        'Color', 'w', 'FontSize', 9, 'FontWeight', 'bold');
                end
            end
        ...
        function s = status_text_(obj, headline)
            cfg = obj.Cfg;
            d_dog = norm(obj.geo.pos_dog - obj.geo.pos_human);
            ...
            s = sprintf([ ...
                ...
                'sample : %s\n' ...
                'fs %d  mics %d  N %d\n' ...
                'dog %.2f m  human %.0f cm  room %dx%dx%dm'], ...
                headline, spname, cfg.fs, cfg.n_mics, cfg.frame_size, ...
                d_dog, cfg.human_height*100, cfg.room_dims(1), cfg.room_dims(2), cfg.room_dims(3));
        end
    ...
    methods (Access = private)
        ...
        %BUILD_MIX_  Mix the loaded speech with factory + welding noise through
        %   the physical array. Caches mic_mix_ (=[L x N]) and play_mix_.
            ...
            factory = obj.factory_gain * obj.audio.next_factory_chunk(L);
            welding = obj.welding_gain * obj.audio.next_welding_chunk(L);

            obj.mixer.reset();
            mic = obj.mixer.mix(speech, factory, welding);           % [L x N]
            obj.mic_mix_ = mic;
    ...
    methods (Access = private)
        ...
        %   the mixer, and the 3-D scene.
            ...
            obj.Cfg.mouth_height            = p.human_h;
            obj.Cfg.slant_dist              = p.slant_dist;
            obj.Cfg.dog.azimuth_deg         = p.dog_az;
            ...
            d_dog = norm(obj.geo.pos_dog - obj.geo.pos_human);
            fprintf('[Dorai] Preset %d: %s — dog at [%.2f %.2f %.2f] m (%.2f m)\n', ...
                    k, p.name, obj.geo.pos_dog, d_dog);
        ...
        %   a rebuild of the per-mic waveform rows.
            ...
            fprintf('[Dorai] Array set: %d mics @ %.0f cm (mixer rebuilt)\n', ...
        ...
        function cb_gain_(obj, src, which)
            if strcmp(which, 'factory')
                obj.factory_gain = src.Value;
                obj.H.lbl_dg.String = sprintf('Factory noise  %.2f', src.Value);
            else
                obj.welding_gain = src.Value;
                obj.H.lbl_eg.String = sprintf('Welding noise  %.2f', src.Value);
            end
        ...
        function cb_clean_(obj, src)
            if logical(src.Value)
                ...
                if isempty(obj.clean_)
                    ...
                    obj.set_status_(obj.status_text_('Running Model...'), ...
                                    [0.95 0.85 0.40]);
                    drawnow;
                    try
                        if obj.energy_on_
    ...
    methods (Access = private)
        ...
        function reset_transport_btn_(~, btn)
            ...
            elseif contains(s, 'Clean')
                btn.String = sprintf('%c Clean Speech', 9658);
            else
