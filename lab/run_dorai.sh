#!/usr/bin/env bash
#
# run_dorai.sh — set up the Python venv for the Dorai acoustic simulator
...
#
# Usage:
#   ./run_dorai.sh                 # full setup + launch run_simulation
#   ./run_dorai.sh some_script     # launch a different .m script instead
#   SKIP_SETUP=1 ./run_dorai.sh    # skip venv/deps, just launch MATLAB
#   MATLAB_BIN=/path/to/matlab ./run_dorai.sh
#
set -euo pipefail
...
if [ ! -x "$MATLAB_BIN" ]; then
    ...
    echo "       Set it, e.g.:  MATLAB_BIN=/Applications/MATLAB_R2025a.app/bin/matlab ./run_dorai.sh" >&2
    exit 1
fi
